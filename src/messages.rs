...
pub fn create_heartbeat(state: &state::Context) -> Heartbeat {
    ...
    Heartbeat {
        ...
        measurement_error: (state.state.measurement_error.unwrap_or(0) != 0) as u8,
        measurement_error_occured: 0,   // TODO: implement
        measurement_error_count: state.state.measurement_error.unwrap_or(0) as u32,
        leak: (state.state.leak.is_some() as u8),
        leak_occured: state.state.leak.unwrap_or(0),
    }
}

pub(crate) fn encode_message(message: MessagePayload) -> Result<([u8; 4096], usize), &'static str> {
...
pub(crate) fn encode_register_message(register: Register) -> ([u8; 4096], usize) {
    let mut buffer = [0; 4096];
    buffer[0..9].copy_from_slice(&encode_header(MessageHeader {
        magic: 0xfafafaff,
        typ: 0x01,
